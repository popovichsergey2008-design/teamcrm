...
export function buildTools(deps: ToolDeps): ToolDef[] {
  ...
  return [
...

    {
      ...
      */
      ...
      async run(ctx, p) {
        ...
        /*
          Ищем ТОЛЬКО по документам, а не фильтруем выдачу после.

          «Как ставятся задачи» тонет в самих задачах со словом «задача»: их в базе
          тысячи, справочника — десяток, и он не попадает даже в первую сотню. Отбор
          на стороне базы это исключает. Заодно это и граница доступа: у регламента
          нет проекта, всё остальное сюда просто не попадает.
        */
        const docs = (await knowledge.search(ctx.tenantId, q, 5, undefined, ['regulation']))
          .filter((h) => h.sourceType === 'regulation');
        if (!docs.length) {
...

    {
      ...
      description: 'Поискать в интернете то, чего НЕТ в ANTHILL: документацию, цены, законы, публичные сведения о компании. По задачам, чатам и митам интернет не нужен — там ищи нашими инструментами.',
      ...
      async run(ctx, p) {
        ...
        if (!key) {
          ...
          return { text: 'Веб-поиск выключен администратором или не настроен ключ — отвечай только по данным ANTHILL.', sources: [] };
        }
