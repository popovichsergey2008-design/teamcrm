 ...
 *
 * Чья это служба. TeamCRM — коробочный продукт, поэтому отвечает ВЕНДОР: обращение
  ...
  private async answerByAi(
    ...
    try {
      ...
      await this.agent.ask(
        ...
        */
        `${question}\n\n`
        + `(Это вопрос в службу заботы TeamCRM. Отвечай коротко и по делу: что нажать и где `
        + `это находится. Про устройство системы отвечай по справочнику TeamCRM из базы знаний `
