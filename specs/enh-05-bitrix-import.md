...
- **E2 (готово).** Вложения задач (Bitrix Disk → MinIO, тип вне whitelist пропускается); **лента проекта → `imported_messages`** (панель «Лента (импорт)» в доске, read-only); ручная привязка несопоставленных пользователей (external_refs(user), учитывается при импорте и исключает из unmatched). Файлы в комментариях — пока не отдельно (комментарии — текст).
- **E3 (живой инкремент — готово).** Исходящий вебхук Битрикса → публичный `POST /api/integrations/bitrix/events/:token` (роутинг по секретному `event_token` подключения, миграция 0013). ONTASKADD/UPDATE/COMMENTADD → `tasks.task.get` → upsert задачи в её проект (если проект импортирован) + колонки/комментарии/вложения/теги; ONTASKDELETE → удаление задачи. Обработка в фоне (быстрый 200 Битриксу). URL события показывается в карточке подключения. Осталось (отдельные этапы): двусторонняя синхронизация (наши изменения → Битрикс), OAuth-приложение, CRM-воронки.
