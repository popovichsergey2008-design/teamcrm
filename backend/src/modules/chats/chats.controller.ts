...
class MessageEditDto {
  /*
    Предел правки — ТОТ ЖЕ, что и у отправки.

    Живая жалоба: «в чате не редактируются сообщения». На снимке ответ сервера:
    «body must be shorter than or equal to 4000 characters». Отправить сообщение
    можно было длиной до 8000 знаков, а поправить в нём опечатку — только до 4000:
    длинный отчёт со ссылками правке не поддавался вовсе.

    Два предела на одно и то же поле — всегда ошибка: править можно ровно то, что
    разрешено написать.
  */
  @IsString() @MaxLength(8000, { message: 'Сообщение длиннее 8000 знаков' }) body!: string;
}
