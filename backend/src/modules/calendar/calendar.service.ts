...
/**
 * Напоминания у события, для которого их не указали.
 *
 * Три, а не одно: за час — успеть подготовиться, за 15 минут — закончить текущее дело,
 * за 5 — дойти. Пустой список означает осознанное «не напоминать» и сюда не попадает.
 */
const DEFAULT_REMINDERS = [5, 15, 60];
/** Шесть — потолок: три по умолчанию плюс то, что человек добавил сам. */
