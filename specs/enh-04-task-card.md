...
## Роли в задаче — исполнитель и руководитель (добавлено 2026-06-30)

- `tasks.assignee_id` — **исполнитель** (кто делает). `tasks.created_by` (миграция `0011`) — **руководитель/постановщик** (по умолчанию = создатель задачи; редактируется через `PATCH /api/tasks/:id {managerId}`).
- Доска (`GET .../board`) обогащает карточки `assignee_name` и `manager_name` (карта id→ФИО по tenant). На канбан-карточке — чип исполнителя (инициал-аватар + имя); в TaskDrawer — селекты «Исполнитель» и «Руководитель» + сводка «Исполнитель: … · Руководитель: …».
- Realtime шлёт «сырую» задачу без имён — фронт резолвит имя исполнителя из загруженного списка `users` как фолбэк (для client-роли имя приходит с сервера на полной загрузке доски).
- **Форма создания задачи** (`TaskCreateModal`): кнопка «+ Задача» открывает модалку с полями название / исполнитель / руководитель (дефолт — текущий пользователь) / описание. `POST /api/tasks` принимает `assigneeId` и `managerId` (если `managerId` не задан — руководитель = создатель). Заменила быстрый инлайн-ввод названия.

## Виды доски: Канбан / Список (добавлено 2026-06-30)

Переключатель вида в шапке проекта (▦ Доска / ☰ Список); выбор запоминается в `localStorage (teamcrm.boardView)`.
- **Доска** — существующий канбан (колонки, drag-and-drop задач/колонок).
- **Список** (`TaskListView`) — задачи сгруппированы по колонкам, компактные строки: риск-светофор, название, метки, приоритет, счётчики (комменты/вложения/чеклист), чип исполнителя (фолбэк имени из `users`), кнопка таймера; клик по строке открывает карточку. Использует те же данные `GET .../board`, без отдельного запроса.
- На будущее (как в популярных CRM): табличный вид с колонками-полями, календарь по дедлайнам, Gantt/таймлайн.
