    ...
    <div className={`mini-call${detached ? ' mini-call-pip' : ''}${tiny ? ' mini-tiny' : ''}`} ref={boxRef}>
      ...
      <div className="mini-bar">
        ...
        {/* Убрать с глаз — не то же самое, что выйти: соседняя красная кнопка кладёт
            трубку, эта только прячет окно, оставляя в углу кнопку с часами.
            С системным крестиком окошка её не путать: тот ВОЗВРАЩАЕТ разговор в полное
            окно, и это разные желания. */}
        {onHide && (
          <button
            className="mini-btn"
            onClick={onHide}
