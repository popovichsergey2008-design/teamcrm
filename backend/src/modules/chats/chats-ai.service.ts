...
export class ChatsAiService {
  ...
  /**
   * Причесать расшифровку голосового (просьба заказчика).
   *
   * Whisper отдаёт поток слов без знаков препинания и заглавных букв: «так константин
   * еще поставь это самое на оплату как ты вот делал…». Читать такое невозможно, и
   * голосовые перестают слушать и писать.
   *
   * Модель здесь НЕ пересказывает и НЕ сокращает: только расставляет знаки, делит на
   * предложения, чинит очевидные ослышки и убирает слова-паразиты. Пересказ был бы
   * хуже исходника — в рабочей переписке важны именно сказанные слова, а решение о
   * формулировке остаётся за автором: сообщение он может поправить как любое своё.
   *
   * Не получилось — возвращаем расшифровку как есть: текст без знаков лучше, чем его
   * отсутствие, и запись из-за недоступности модели терять нельзя.
   */
  async tidyTranscript(tenantId: string, text: string): Promise<string> {
    const raw = String(text ?? '').trim();
    if (raw.length < 12) return raw;
    try {
      const out = await this.ai.generate(tenantId, TIDY_SYSTEM, JSON.stringify({ text: raw.slice(0, 6000) }), 'chat_voice_tidy');
      const clean = String(out ?? '').trim().replace(/^«|»$/g, '');
      if (!clean) return raw;
      /*
        Страховка от пересказа: если модель вернула вдвое короче сказанного, она не
        причесала, а пересказала — берём исходную расшифровку. Потерять половину
        поручения хуже, чем прочитать его без запятых.
      */
      if (clean.length < raw.length * 0.5) return raw;
      return clean.slice(0, 4000);
    } catch (e) {
      this.log.warn(`причёсывание расшифровки: ${(e as Error).message}`);
      return raw;
    }
  }
...
const TIDY_SYSTEM = [
  'Ты приводишь в порядок расшифровку голосового сообщения.',
  'Верни ТОТ ЖЕ текст: расставь знаки препинания и заглавные буквы, раздели на предложения и абзацы.',
  'Исправь очевидные ослышки распознавания (имена, названия, числа) — только если исправление очевидно из контекста.',
  'Убери слова-паразиты и повторы («это самое», «ну», «вот»), если без них смысл не меняется.',
  'НИЧЕГО не сокращай, не пересказывай и не добавляй от себя. Не отвечай на сообщение.',
  'Сохрани язык оригинала. В ответе — только готовый текст, без пояснений и кавычек.',
].join(' ');
