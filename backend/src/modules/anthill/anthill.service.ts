...
export class AnthillService {
  ...
  async ask(
    ...
    */
    ...
    /*
      Пустой план — не повод отказывать человеку.

      «Как ставятся задачи?» планировщик считал разговором без данных и не звал
      ничего; ответчик, которому велено говорить только по найденному, честно выдавал
      «не нашёл подтверждения в данных ANTHILL» — при том, что ответ лежал в
...
const NEXT_WAVE_SYSTEM = [
  'Ты ведёшь глубокий разбор по данным ANTHILL. Тебе дали вопрос и то, что уже найдено.',
...
const DEEP_SYSTEM = [
  'Ты — AnthillBot, персональный AI-помощник в ANTHILL. Это ГЛУБОКИЙ РАЗБОР: человек ждёт отчёт, а не реплику.',
...
const PLAN_SYSTEM = [
  'Ты — планировщик AnthillBot, помощника в ANTHILL. По вопросу человека реши, какие инструменты вызвать.',
